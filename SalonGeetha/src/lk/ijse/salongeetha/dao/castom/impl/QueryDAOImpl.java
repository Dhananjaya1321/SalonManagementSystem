...
public class QueryDAOImpl implements QueryDAO {
    ...
    public ResultSet getAmountDueServiceAppointmentDetails(ServiceAppointmentDetailDTO serviceAppointmentDetailDTO) throws SQLException, ClassNotFoundException {
        return CrudUtil.setQuery("select s.price,s.discount from service_appointment_detail ad join service s on ad.sev_Id = s.sev_Id where Apt_Id=?", serviceAppointmentDetailDTO.getAptId());
    }
    ...
    public ResultSet getAmountDueBookRentalsDetail(BookRentalsDetailDTO bookRentalsDetailDTO) throws SQLException, ClassNotFoundException {
        return CrudUtil.setQuery("select bd.Qty,bd.For_how_many_days,r.Price_pre_day,r.Discount \n" +
                "from book_rentals_detail bd join rentals r on bd.Rent_Id = r.Rent_Id where Bok_Id = ?", bookRentalsDetailDTO.getBokId());
    }
}
